...
unittest
{
    ...
    if (auto m = regexp.search("abcdef", "c()"))
    {
    ...
    if (auto n = regexp.search("abcdef", "g"))
    {
    assert(0);
    }
}
...
RegExp rexSearch(rchar[] pattern, rchar[] attributes = "")(rchar[] string)
{
    RegExp reg = new RegExp(pattern, attributes);
    const uint attrib = RegExp.metaCompileAttributes!(attributes);
    static assert(attrib==0);

    reg.input = string;
    reg.pmatch[0].rm_eo = 0;
    return reg;
}
...
class RegExp
{
    ...
    public this(rchar[] rawpattern, rchar[] rawattributes, ubyte [] precompiledprogram, uint precompiledattributes, uint precompiledCaptures)
    {
        input = null;
        pmatch = (&gmatch)[0 .. 1];
        this.pattern = rawpattern;
        this.flags = rawattributes;

        attributes = precompiledattributes;
        re_nsub = precompiledCaptures;
    ...
    int i;
    foreach(m; rexSearch!("ab")("abcabcabab"))
    {
    ...
    rchar[] pattern;        // source text of the regular expression. used for optimisation in replace()
...
private static int isword(dchar c) { return isalnum(c) || c == '_'; }

private const uint inf = ~0u;


/* =================== Compiler ================== */
// Compilation involves setting 3 values:
// the compiled program ('program'),
// the compiled attributes ('attributes'),
// and the number of possible captures ('re_nsub')

/* ********************************
 * Throws RegExpException on error
 */

public void compile(rchar[] pattern, rchar[] patternattributes)
{
    this.flags = patternattributes;

    uint oldre_nsub = re_nsub;
    uint numCaptures = 0;
    this.attributes = compileAttributes(patternattributes);

    program = compilePattern(pattern, numCaptures, attributes);
    re_nsub = numCaptures;
...
template metaCompileAttributes(rchar[] patternattributes, uint sofar = 0)
{
    static if (patternattributes.length==0) {
        const uint metaCompileAttributes = sofar;
    } else static if (patternattributes[0]=='g') {
        static assert(!(sofar & REA.global), "Regexp error: Redundant attribute");
        const uint metaCompileAttributes = metaCompileAttributes!(patternattributes[1..$], sofar | REA.global);
    } else static if (patternattributes[0]=='i') {
        static assert(!(sofar & REA.ignoreCase), "Regexp error: Redundant attribute");
        const uint metaCompileAttributes = metaCompileAttributes!(patternattributes[1..$], sofar | REA.ignoreCase);
    } else static if (patternattributes[0]=='m') {
        static assert(!(sofar & REA.multiline), "Regexp error: Redundant attribute");
        const uint metaCompileAttributes = metaCompileAttributes!(patternattributes[1..$], sofar | REA.multiline);
    } else static assert(0, "Regexp error: unrecognized attribute: " ~ patternattributes[0]);

}

unittest {
    static assert(RegExp.metaCompileAttributes!("mig")==7);
}

// p is following the \ char
// none of the cases are multibyte
template escapechar(char [] pattern, int p)
{
    static if (pattern[p]=='b') const dchar escapechar = '\b';
    else static if (pattern[p]=='f') const dchar escapechar = '\f';
    else static if (pattern[p]=='n') const dchar escapechar = '\n';
    else static if (pattern[p]=='r') const dchar escapechar = '\r';
    else static if (pattern[p]=='t') const dchar escapechar = '\t';
    else static if (pattern[p]=='v') const dchar escapechar = '\v';
    else static if (pattern[p]=='c') {
        static if (pattern.length==p+1) const dchar escapechar = 'c';
        // Note: we are deliberately not allowing dchar letters
        else static if ((('a' <= pattern[p+1] && pattern[p+1] <= 'z')
            || ('A' <= pattern[p+1] && pattern[p+1] <= 'Z'))) {
                const dchar escapechar = pattern[p+1] & 0x1F;
        } else static assert(0, `letter expected following \c`);
    } else static if (pattern[p]>='0' && pattern[p]<='7') {
        // who uses octal???
    } else static if (pattern[p]=='x') {
    } else static if (pattern[p]=='u') {
    } else const dchar escapechar = pattern[p];
}

static uint compileAttributes(rchar[] patternattributes)
{
    uint attributes = 0;
    foreach (rchar c; patternattributes)
    {   REA att;
    ...
    switch (c)
    {
        ...
        default:
        throw new RegExpException("unrecognized attribute");
        assert(0);
    }
    if (attributes & att)
    {   throw new RegExpException("redundant attribute");
        assert(0);
    }
    attributes |= att;
    }
...
static ubyte[] compilePattern(rchar[] pattern, out uint re_nsub, uint attributes)
{
   OutBuffer buf;
