...
{
    static if (str.length>0 && isdigit!( (str[0]) ))
...
{
    static if (str.length>1 &&  isdigit!(( str[0] )) ) {
        static if (getLnameConsumed!(str) < str.length && isdigit!(( str[getLnameConsumed!(str)] ))) {
...
template templateValueArgConsumed(char [] str)
{
    ...
    else static if (isdigit!( (str[0]) )) const int templateValueArgConsumed = countLeadingDigits!(str);
...
template prettyValueArg(char [] str)
{
    ...
    else static if (isdigit!( (str[0]) )) const char [] prettyValueArg = str;
