 ...
 *	Thomas Kuehne
 * Reworked for DDL/Tango by Eric Anderton
 */
...
char[] demangle(char[] name)
{
	...
	switch (name[ni++])
	{
		...
		case 'M':               // D member function
			assert(name[ni] == 'F',"next char is invalid: " ~ name[ni]);
			ni++;
			/* fallthrough */
	    case 'F':				// D function
	    case 'U':				// C function
		...
		case 'Z':
			assert(ni == name.length); // must be at the end if a metadata symbol
			//TODO: flag something?
			goto L1;
